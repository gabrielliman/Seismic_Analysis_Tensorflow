...
def adaptable_attentionunet(input_shape, filters=[16, 32, 64, 128, 256], kernelsize=3, dropout=0.2, upsample_size=2, batchnorm=True):
    ...
    # Downsampling layers
    dn = []
    dn.append(conv_block(inputs, kernelsize, filters[0], dropout, batchnorm))
    if numfilters > 1:
        pool = layers.MaxPooling2D(pool_size=(2, 2))(dn[0])
        for i in range(1, numfilters):
            dn.append(conv_block(pool, kernelsize, filters[i], dropout, batchnorm))
    ...
    if numfilters > 1:
        for pos in range(1, numfilters):
            ...
            up_conv = conv_block(up, kernelsize, filters[pos-1], dropout, batchnorm)  # Fix here
