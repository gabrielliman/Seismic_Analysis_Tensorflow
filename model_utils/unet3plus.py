...
def unet_3plus_2d_base(input_tensor, filter_num_down, filter_num_skip, filter_num_aggregate, dropout_rate=0, kernel_size=3,
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    if backbone is None:
        ...
        X = CONV_stack(X, filter_num_down[0], dropout_rate=dropout_rate,kernel_size=kernel_size, stack_num=stack_num_down, 
        ...
        for i, f in enumerate(filter_num_down[1:]):
            ...
            X = UNET_left(X, f,dropout_rate=dropout_rate,kernel_size=kernel_size, stack_num=stack_num_down, activation=activation, 
        ...
        if depth_encode < depth_:
            ...
            for i in range(depth_-depth_encode):
                ...
                X = UNET_left(X, filter_num_down[i_real], dropout_rate=dropout_rate,kernel_size=kernel_size, stack_num=stack_num_down, activation=activation, pool=pool, 
    ...
    for i in range(depth_decode):
        ...
        for lev in range(depth_decode):
            ...
            X = CONV_stack(X, f, dropout_rate=dropout_rate,kernel_size=kernel_size, stack_num=1, 
        ...
        X = CONV_stack(X, filter_num_aggregate, dropout_rate=dropout_rate,kernel_size=kernel_size, stack_num=stack_num_up, 
    ...
    if depth_decode < depth_-1:
        for i in range(depth_-depth_decode-1):
            ...
            X = UNET_right(X, None, filter_num_aggregate, dropout_rate=dropout_rate,kernel_size=kernel_size, stack_num=stack_num_up, activation=activation, 
...
def my_unet_3plus_2d(input_size, n_labels, filter_num_down,dropout_rate=0,kernel_size=3, filter_num_skip='auto', filter_num_aggregate='auto', 
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    X_decoder = unet_3plus_2d_base(IN, filter_num_down, filter_num_skip, filter_num_aggregate,  dropout_rate=dropout_rate,kernel_size=kernel_size,
    ...
    if deep_supervision:
        ...
        for i in range(1, L_out):
            ...
            X = layers.Conv2D(n_labels, kernel_size, padding='same', name='{}_output_conv_{}'.format(name, i-1))(X_decoder[i])
            ...
            if output_activation:
                ...
                if output_activation == 'Sigmoid':
                    X = layers.Activation('sigmoid', name='{}_output_sup{}_activation'.format(name, i-1))(X)
                else:
        ...
        X = CONV_output(X_decoder[0], n_labels, kernel_size=kernel_size, 
        ...
        model = models.Model([IN,], OUT_stack)

    else:
        OUT = CONV_output(X_decoder[0], n_labels, kernel_size=kernel_size, 
                          activation=output_activation, name='{}_output_final'.format(name))

        model = models.Model([IN,], [OUT,])
