...
if __name__ == '__main__':
    ...
    train_image,train_label, test_image, test_label, val_image, val_label=article_division_data(shape=(992,576), stridetrain=(256,64), strideval=(256,64))
    ...
    tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(model_log_dir, 'articledivision_model.keras'),
    ...
    history = model.fit(train_image, train_label, steps_per_epoch=steps_per_epoch, batch_size=args.batch_size, epochs=args.epochs,
                        callbacks=callbacks,validation_data=(val_image, val_label))     
    ...
    try:
        model.load_weights(os.path.join(model_log_dir, 'articledivision_model.keras'))
    finally:
    #model.summary()
        model.save("/scratch/nunes/seismic_models/"+"article_model"+"_"+"article_division"+".keras")
    ...
    make_prediction("article_model_article_division","parihaka", model, test_image, test_label, num_classes=6)

    #TO COM PROBLEMA DE MEMORIA