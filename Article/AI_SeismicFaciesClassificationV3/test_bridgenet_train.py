...
parser.add_argument('--patch_cols', default=192, type=int, help='patch_cols')
parser.add_argument('--stride', default=[128, 64], type=int, help='stride', nargs='*')
...
if __name__ == '__main__':
    ...
    tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(model_log_dir, 'mydivision_model992x192.keras'),
    ...
    try:
        model.load_weights(os.path.join(model_log_dir, 'mydivision_model992x192.keras'))
    finally:
    #model.summary()
        model.save("/scratch/nunes/seismic_models/article_model_my_division992x192.keras")
    ...
    make_prediction("article_model_my_division992x192","parihaka", model, test_image, test_label, num_classes=6)